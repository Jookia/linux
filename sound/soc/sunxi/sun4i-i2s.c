...
#define SYSCLK_TYPE_SUN4I 0
#define SYSCLK_TYPE_SUN8I 1
...
static int sun4i_i2s_dai_startup(struct snd_pcm_substream *sub, struct snd_soc_dai *dai)
{
	...
	ret = snd_pcm_hw_constraint_minmax(runtime, SNDRV_PCM_HW_PARAM_CHANNELS,
					   2, i2s->variant->channels_max);
	if (ret < 0) {
		dev_err(dai->dev, "Failed to constrain channels: %d\n", ret);
		return ret;
	}
...
static struct snd_soc_dai_driver sun4i_i2s_dai = {
	.capture = {
		.stream_name = "Capture",
		.channels_min = 1,
		.channels_max = 128, /* Set by variant */
		.rate_min = 8000,
		.rate_max = 384000,
